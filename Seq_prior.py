...
= Vocabulary =
 * a Seq: a Seq is a 'pipeable' function, somthing that you can pipe to,
           In the code '[1, 2, 3] >> add' add is a Seq
 * a Seq function: A standard function returning a Seq so it can be used like
           a normal Seq but called like in : [1, 2, 3] >> concat("#")


= Syntax =
The basic symtax is to use a Seq like in a shell :
>>> [1, 2, 3] >> add
6

A Seq can be a function call, for exemple the Seq function 'where' :
>>> [1, 2, 3] >> where(lambda x: x % 2 == 0) #doctest: +ELLIPSIS
<generator object <genexpr> at ...>

A Seq as a function is nothing more than a function returning
a specialized Seq.


= Constructing your own =
You can construct your pipes using Seq classe initialized with lambdas like :

stdout = Seq(lambda x: sys.stdout.write(str(x)))
...
__version__ = '1.4'
__all__ = [
    'Seq', 'take', 'tail', 'skip', 'all', 'any', 'average', 'count',
